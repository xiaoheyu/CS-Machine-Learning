...
def entropy(rows):
    ...
    for lbl in counts:
        prob_of_lbl = counts[lbl] / float(len(rows))
        E -= prob_of_lbl*math.log2(prob_of_lbl)
    return E
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.predictedLabel = max(self.predictions,key=self.predictions.get)
        self.id = id
        self.depth = depth
...
def print_tree(node, spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        print (spacing + "Predict", node.predictions, "Predicted Label: "+ node.predictedLabel)
        return

    # Print the question at this node
    print (spacing + str(node.question) + "  " + "node depth:" + str(node.depth) + "  " + "node id:" + str(node.id))
